<?php 
    ...
    $this->db->where('back_date_invoice_id', $id);  // Also mention table name here
    $query = $this->db->get(); 
	$seleted_date=$query->result_array();
	//echo "<pre>";
	//print_r($seleted_date);die;
	foreach($seleted_date as $value)
	{
		$string.='"'.date('j-n-Y',strtotime($value['date_range'])).'",';
	}
	$string=rtrim($string,",");
    $seleted_date='['.$string.']';
				...
				<div class="box col-lg-12 col-md-12 col-xs-12 " style="margin-top:-10px;">
					<div class="box-inner">
						<div class="box-header well">
							<h2>Back Date Invoice</h2>
							
							
							<input type="text" style="display:none;" class="form-control " id="Access" name="Access" />
							<input type="text" style="display:none;" class="form-control" id="Validity" name="Validity" />
						</div>
						<div class="box-content">
							<form   method="post" action="<?php echo base_url('BackDateInvoice/saveInvoiceData'); ?>"  id="form_sales_invoice" onsubmit="return checkInvoiceForm();" enctype="multipart/form-data">
							<div class="row">
								<div class="form-group col-lg-3">
									<label class="control-label">Manual Invoice Date</label>
									<input type="text" class="form-control" onChange="products();" id="sales_invoice_date" name="invoice_date" value="<?php //echo date('d/m/Y H:i:s');?>"/>
								</div>
								<div class="form-group col-lg-3" >
									<label class="control-label">Type of Invoice</label>
									<div class="controls">
										<select id="invoice_type"  name="invoice_type" data-rel="chosen" style="width:70%;">
											<option value="backdatesale">Sale</option>
											<!--<option value="backdatecredit">Credit</option>
											<option value="backdateproforma">Proforma</option>
											<option value="backdatecorporate gift">Corporate Gift</option>-->
											<option value="backdateadvance">Advance Mode</option>
										</select>
									</div>
								</div>
								<div class="form-group col-lg-3">
									<label class="control-label">Narration</label>
									<input type="text" class="form-control" id="customer_narration" name="customer_narration" />
								</div>
								<div class="form-group col-lg-3">
									<label class="control-label">Manual Invoice No. </label>
									<input type="text" class="form-control" id="customer_reference_number" name="customer_reference_number" />
								</div>
							...
							<div class="row">
								...
								<div class="form-group col-lg-3">
									...
									<input type="text" class="form-control" id="customer_pan_number" maxlength="10" style="text-transform:uppercase" name="customer_pan_number"/>
								</div>
...
$initialStock = isset($product_current_stock->product_current_stock) ? $product_current_stock->product_current_stock : '0';
						if($initialStock > '0'){
								...
								<div class="form-group col-lg-2">
									<input type="text" class="form-control" id="price_rate-0" maxlength="6" value="" name="rate_per_quantity[]" readonly />
								</div>
								<div class="form-group col-lg-1">
									<input type="text" class="form-control" name="discount_percent[]" id="discount_percent-0" onblur="getNetAmount(this.id,'0','dis');" readonly="" readonly="" />
								</div>
								<div class="form-group col-lg-1">
									<input type="text" class="form-control" id="tax-0" name="tax[]" readonly />
								</div>
								<div class="form-group col-lg-2" id="serial_number_div0">
						...
						<!--	<div class="row">
							...
							<div class="row form-group col-lg-12" style="padding:0;" id="sales_invoice_payment_mode-0">
								...
								<div class="form-group col-lg-3" style="width:140px;">
									...
									<input type="text" style="width:120px;" class="form-control" name="payment_mode_amount[]" id="payment_mode_amount_0" onchange="getTotal_received();"  />
								</div>
								
								<div class="form-group col-lg-3" id="div_card_data_0" style="display:none;">
									<label class="control-label" id="lable_card_0">Card Number</label>
							...
							<hr/>
							<div class="row">
								<div class="form-group col-lg-3 col-lg-push-0" style="width:260px;">
									<label class="control-label">Amount Received (Rs.) :</label>
									<input type="text" class="form-control" readonly name="received_amount" id="received_amount" onkeypress="getAmountReceived();" />
								</div>
								<div class="form-group col-lg-3">
									<label class="control-label">Upload Document</label>
									<input type="file"  id="invoice_upload_document" name="invoice_upload_document" />
								</div>
								<div class="form-group col-lg-3">
							...
							<input type="hidden" name="selected_products_divs" id="selected_products_divs" value="" />
								...
								<div class="form-group col-lg-2 col-lg-push-4" style="width:14% !important;">
									<button type="button" class="btn btn-primary" onclick="javascript:window.location.href='<?php echo base_url('BackDateInvoice/back_date_sales_invoice_details');?>';">Cancel</button> 
			...
			<div class="modal-body">			
				<div class="box-content">
					...
					<div class="form-group col-lg-4">
						...
						<div class="form-group col-lg-4">
							<label class="control-label">PAN Number</label>
							<input type="text" class="form-control"  id="modal_customer_pan_number" maxlength="10" style="text-transform:uppercase" name="modal_customer_pan_number" value="<?php echo set_value('modal_customer_pan_number'); ?>" />
...
function TotalProductHidden()
	...
	$(document).ready(function(){
		...
		var availableDates = <?php echo $seleted_date; ?>;
	...
	if(data_value>0)
	{
		bootbox.confirm("Extra Received Amount is "+data_value + " .  Do you want to proceed  ?" ,function(result){
		if(result == true){
		$.ajax({
		...
		success : function (res){
			...
			if(res.msg=='success'){
				...
				window.location.href='<?php echo base_url('invoice/sales_invoice_receipt');?>?backdate=1&invoice_id='+res.invoice_id;
			}			
			else {
...
netAmount = parseFloat($("#total_net_amount").val());
	...
	TotalProductHidden();
		...
		if((amountReceived < netAmount) && $("#invoice_type :selected").val()!='backdateadvance'){
...
//}); 
	...
	function getPrice(id,net_stock_id){
		...
		datevalue=$('#sales_invoice_date').val();
		if(datevalue=="")
		{
			alert("Please Setect Date First");
			return false;
		}
		...
		$.ajax({
				url : "<?php echo base_url(); ?>BackDateInvoice/getPriceByProduct",
				type : "post",
				data : {product_id:product_id,back_date:datevalue},
				success : function(res){ 
	...
	function addNewProductSalesInvoice()
	{
        ...
        invoice_date=$('#sales_invoice_date').val();
		$.ajax({
				url : "<?php echo base_url();?>invoice/AjaxAddNewDivCommon",
				type: "POST",
				data: {invoice_date_value:invoice_date,divSize:divSize,already_products:already_products,pageName:"back_date_sales_invoice_productsdiv"},
				success: function(res){
					if(divSize=='0'){
...
function products()
{
	invoice_date=$('#sales_invoice_date').val();
	$.ajax({
					url : "<?php echo base_url();?>BackDateInvoice/productsbydate",
					type: "POST",
					data: {incoice_date_value:invoice_date,pageName:"back_date_sales_invoice_products"},
					success: function(res){
					
							//alert(res);
							$('#product_div_id_0').html(res);
					}
	});
